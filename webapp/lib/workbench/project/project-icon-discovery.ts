/*
 * Exports:
 * - discoverWorkbenchProjectIcon: choose one preferred Git-visible PNG or ICO across ordered project roots. Keywords: project, icon, favicon, git, workspace, discovery.
 * - selectWorkbenchProjectIcon: deterministically rank already-listed icon candidates for focused tests and callers. Keywords: project, icon, preference, path, root.
 */
...
export async function discoverWorkbenchProjectIcon(
  ...
  const pathsByRoot = await Promise.all(
    roots.map(async ({ rootPath }) => (
      await hasGitMarker(rootPath)
        ? await listGitVisibleFiles(rootPath, PROJECT_ICON_PATHSPECS)
        : []
    )),
  );
