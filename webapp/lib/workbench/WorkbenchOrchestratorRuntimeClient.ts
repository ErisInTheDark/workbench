/*
 * Exports:
 * - WorkbenchOrchestratorRuntimeClientOptions: shared-socket request seam for browser reload runtime tests. Keywords: reload, WebSocket, browser, test.
 * - default WorkbenchOrchestratorRuntimeClient: own ordered orchestrator reload dirt, mixed-version fallback, and user reload admission. Keywords: reload, revision, reconnect, lifecycle.
 */
import type {
...
import {
  ...
  WorkbenchOrchestratorReloadDirtEnvelopeSchema,
...
export interface WorkbenchOrchestratorRuntimeClientOptions {
...
export default class WorkbenchOrchestratorRuntimeClient {
  #dedicatedObservation = false;
  #disposed = false;
  readonly #listeners = new Set<() => void>();
  readonly #request: WorkbenchOrchestratorRuntimeClientOptions["request"];
  #revision = -1;
  #snapshot: WorkbenchReloadDirtSnapshot = EMPTY;

  constructor({ request }: WorkbenchOrchestratorRuntimeClientOptions) {
  ...
  async open() {
    try {
      ...
      return this.acceptEnvelope(response, "Rejected Workbench orchestrator reload dirt response");
    } catch (error) {
      ...
      this.#publish({
        ...this.#snapshot,
        error: error instanceof Error ? error.message.slice(0, 500) : "Workbench orchestrator reload observation failed.",
      });
      throw error;
    }
  }
  ...
  acceptUpdate(value: unknown) {
    return this.acceptEnvelope(value, "Rejected Workbench orchestrator reload dirt update");
  }
  ...
  private acceptEnvelope(value: unknown, message: string) {
    ...
    const parsed = WorkbenchOrchestratorReloadDirtEnvelopeSchema.safeParse(value);
    if (!parsed.success) {
