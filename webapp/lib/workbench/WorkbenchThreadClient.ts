...
} from "./thread/workbench-thread-page";
...
function WorkbenchThreadClient(
  ...
  let installedSidebarSnapshot: WorkbenchThreadSidebarSnapshot | null = null;
  ...
  function installSidebarSnapshot(snapshot: WorkbenchThreadSidebarSnapshot | null) {
    installedSidebarSnapshot = snapshot;
    if (!snapshot) {
      state.subagents = [];
      state.threads = [];
      state.threadsError = "";
    ...
    state.threads = snapshot.entries.flatMap((entry): ThreadSummary[] => {
  ...
  function replacePendingUserInputRequests(
    ...
    for (const entry of installedSidebarSnapshot?.entries ?? []) {
  ...
  async function fetchThreadPayload(
    ...
    try {
      ...
      const nextModel = isCurrentThread
        ? currentModel
        : pageResponse.model ?? readStoredHarnessModel(harness);
      ...
      const result: ThreadReadResult = {
        ...
        payload: toThreadPayload(
          ...
          nextModel,
          isCurrentThread
            ? currentReasoningEffort ?? readStoredHarnessModelEffort(harness, nextModel) ?? pageResponse.reasoningEffort ?? null
            : readStoredHarnessModelEffort(harness, nextModel) ?? pageResponse.reasoningEffort ?? null,
          nextServiceTier,
          selectedAgentPath,
          null,
          pageResponse.nextCursor,
        ),
        serviceTierToPersist: harness === "codex" && cursor === null && !hasServiceTierPreference
  ...
  async function sendThreadMessage(
    ...
    let resumedThread = bootstrapThread ?? (canUseProvidedThread ? thread : null);
