/*
 * No production exports. Tests protect ordered reload dirt, reconnect reset, mixed-version fallback, and user reload admission. Keywords: reload, browser, revision, reconnect, test.
 */
import assert from "node:assert/strict";
import { test } from "node:test";

import { WorkbenchDaemonRequestError } from "./daemon/WorkbenchDaemonClient.ts";
import {
  WORKBENCH_RELOAD_DIRT_READ_METHOD,
  WORKBENCH_RELOAD_METHOD,
} from "./orchestrator-reload.ts";
import WorkbenchOrchestratorRuntimeClient from "./WorkbenchOrchestratorRuntimeClient.ts";
...
test("a newer notification cannot be rolled back by an older bootstrap response", async () => {
  ...
  const client = new WorkbenchOrchestratorRuntimeClient({
