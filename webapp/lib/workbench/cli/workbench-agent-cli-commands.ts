...
const LEGACY_CHECKPOINT_MIGRATION_GUIDE = [
  ...
  "10. Release every clean claim without changing files: wb git arc release. Use --disown only to release dirty ownership explicitly.",
  "11. Record successors returned by add, adopt, mv, remove, or continue for later continuation. Final clean removal releases the arc without an active successor.",
  "12. Summarize or inspect the active arc: wb git arc compare [-- <path> [<path>...]] / wb git arc diff [-- <path> [<path>...]]",
  "13. Propose a normal commit: wb git arc propose [--root <root-id>] -m <fresh-title> [-m <optional-description>] [-- <claimed-path> [<claimed-path>...]]",
  "14. Propose message changes to an exact accepted commit without an active arc: wb git arc propose --amend <proposal-id> -m <replacement-title> [-m <replacement-description>]",
  "15. Amend current unpushed HEAD content from the active arc: wb git arc propose --amend [-m <replacement-title> [-m <replacement-description>]] -- <claimed-path> [...]",
  "16. Use the same arc continue command after a proposal is committed and before follow-up work.",
  "17. Restore selected paths: wb git arc restore --ref <ref> -- <path> [<path>...]",
  "18. Restore the full arc only after explicit user direction: wb git arc restore --ref <ref> --confirm",
  "",
...
const HELP_GROUPS: readonly HelpGroupDefinition[] = [
  ...
  {
    commandOrder: ["git arc plan", "git arc start", "git arc continue", "git arc add", "git arc mv", "git arc remove", "git arc release", "git arc compare", "git arc diff", "git arc propose", "git arc restore"],
    footer: [
