...
After context compaction, call `mcp__wbex__thread_recall` before continuing.
...
Use `mcp__wbex__thread_recall_search` for targeted lookup across the complete visible narrative transcript.

Search results are newest-first pages with stable refs and an exact older-results cursor. Read one complete result through fixed-budget pages with `mcp__wbex__thread_recall_expand`.
