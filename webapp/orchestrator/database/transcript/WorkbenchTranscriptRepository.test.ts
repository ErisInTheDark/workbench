...
test("transcript reset clears transcript roots and preserves unrelated database domains", () => {
  ...
  try {
    ...
    database.prepare(`
      INSERT INTO transcript_native_records(
        link_kind,harness_id,native_location,record_kind,payload_json,recorded_at
      ) VALUES ('orphan','codex','C:/project','event','{}',4)
    `).run();
    database.prepare(`
      INSERT INTO transcript_assets(digest,mime_type,byte_length,storage_key,created_at)
      VALUES ('digest','text/plain',1,'asset',5)
    `).run();
    database.prepare(`
      INSERT INTO codex_sandbox_network_global_settings(id,enabled)
      VALUES ('global',1)
    `).run();
    database.prepare(`
      INSERT INTO codex_sandbox_network_project_overrides(project_id,enabled)
      VALUES ('project',0)
    `).run();

    repository.reset();

    assert.equal(repository.read({ threadId: "thread", turnLimit: 1 }), null);
    assert.equal(
      (database.prepare("SELECT COUNT(*) AS count FROM transcript_native_records").get() as { count: number }).count,
      0,
    );
    assert.equal(
      (database.prepare("SELECT COUNT(*) AS count FROM transcript_assets").get() as { count: number }).count,
      0,
    );
    assert.equal(
      (database.prepare("SELECT COUNT(*) AS count FROM workbench_harnesses").get() as { count: number }).count,
      0,
    );
    assert.deepEqual(
      database.prepare("SELECT id,enabled FROM codex_sandbox_network_global_settings").all(),
      [{ id: "global", enabled: 1 }],
    );
    assert.deepEqual(
      database.prepare("SELECT project_id,enabled FROM codex_sandbox_network_project_overrides").all(),
      [{ project_id: "project", enabled: 0 }],
    );
  } finally {
    database.close();
  }
...
test("transcript reset rolls every root deletion back when one delete fails", () => {
  const { database, repository } = createRepository();
  try {
    repository.settle([canonicalWindow([
      threadObservation(),
    ...
    assert.throws(() => repository.reset(), /asset delete failed/u);
    assert.ok(repository.read({ threadId: "thread", turnLimit: 1 }));
    assert.equal(
      (database.prepare("SELECT COUNT(*) AS count FROM transcript_assets").get() as { count: number }).count,
      1,
    );
