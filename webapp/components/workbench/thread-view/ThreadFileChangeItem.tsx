/*
 * Exports:
 * - default ThreadFileChangeItem: render fileChange items with per-file counts and expandable unified diffs. Keywords: workbench, thread, file change, diff.
...
export default function ThreadFileChangeItem ({
  ...
  const parsedChanges: ParsedFileChange[] = item.changes.map((change) => ({
    change,
  ...
  return (
    <div className="space-y-1.5 py-2">
      {parsedChanges.length ? parsedChanges.map((change, index) => (
        <ThreadDisclosure
          key={`${item.id}:change:${index}:${change.change.path}`}
          className="py-0.5"
