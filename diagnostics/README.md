...
Each input is optional; its check skips when absent. Uses no providers, authentication, sandbox setup or running services. Database replay upgrades a private SQLite backup, verifies identities across reopen and projects materialised content. History replay runs separate compatibility-import and provider-settlement cases on independent copies, each twice through production identity admission, full page-response mapping and SQLite projection. Provider settlement precedes body reads, exercising retained identity conversion without relying on a prior browser read. When both inputs are supplied, history replay seeds its private database from the preserved snapshot, exercising existing identities rather than only empty storage. Reports include stage timings for cold and repeated admission; nested timings overlap and must not be summed.
...
Checks startup, profile/instruction admission and managed CLI/MCP identity. Protocol-3 events pass through the real socket client and projection controller; legacy snapshot delivery fails. A fixture-owned command gate holds the live turn while deselection/reselection verifies complete earlier commentary. The completed live items must match durable SQLite identities, order and content before historical reads. Legacy stored transcript/context/cursor reads throw and record attempts in the private source copy, including across SQL page reads and cold reopening; JSON writes remain unchanged. Reopening after owned process-tree retirement must preserve visible items. This does not prove React painting, questionnaire or Browse recording.
