...
5. Still in this same turn, you will now completely implement the plan. Limit the scope of patches — patches often fail if they're too large and will cost a lot of time to re-attempt.
6. Run the "typecheck" task to verify you haven't left any broken code.
7. After completing implementation, it's time for the review pass: Summarise the changes to the user as commentary and then use the ask_questions tool for a confirmation from them. The options you must give the user:
