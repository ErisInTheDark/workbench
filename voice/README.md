...
Build/test modes prepare dependencies automatically. Downloads/builds live in `.workbench/native-voice/`; Cargo outputs live in `voice/target/`. Extraction uses CMake's archive support. Incomplete extraction fails visibly and is retained for inspection. No automatic cleanup or toolchain installation.
...
````sh
cargo test --manifest-path voice/Cargo.toml --lib --locked
````
...
Set `WORKBENCH_VOICE_MODEL_DIR` to the model directory printed by `--prepare`. Run `voice/target/release/workbench-voice` (`.exe` on Windows). Requests/events are newline-delimited JSON over stdin/stdout. Diagnostics use stderr.
