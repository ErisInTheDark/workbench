/*
 * Keywords: thread identity, alias, reconnect, request ownership.
...
test("alias resolution coalesces reads, reuses canonical identity and cannot repopulate a reset connection", async () => {
  ...
  const alias = { projectId: "project", threadId: "provider-thread" };
  const canonical = { ...alias, harness: "codex" as const, threadId: "68054624-3939-4c53-8b92-ac72e6c8024a" };
  ...
  assert.equal((await controller.resolve({ threadId: canonical.threadId, projectId: "project" }))?.harness, "opencode");
  assert.equal(pending.length, 2);
  controller.dispose();
...
test("unobserved identity is not permanently cached", async () => {
  ...
  const controller = new ThreadIdentityController(async () => {
    reads += 1;
    return reads === 1 ? null : { threadId: "68054624-3939-4c53-8b92-ac72e6c8024a", projectId: "project", harness: "codex" };
  });
  const request = { threadId: "legacy" };
