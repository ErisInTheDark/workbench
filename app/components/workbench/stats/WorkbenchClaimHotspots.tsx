  ...
  return (
    <section aria-labelledby="claims-heading" className="space-y-5" data-thread-project-file-link-boundary="true">
      ...
      <div className="h-[40rem] lg:h-[20rem]">
              ...
              return (
                <li className="flex min-w-0 items-baseline justify-between gap-4" key={`${hotspot.projectId}:${hotspot.rootId}:${hotspot.path}`}>
                  <span className="flex min-w-0 items-baseline gap-2 text-[0.76rem]">
                    {global ? <span className="truncate text-muted">{projectNamesById.get(hotspot.projectId) ?? hotspot.projectId}</span> : null}
                    <ProjectFilePath
        ...
        ) : <p className="m-0 text-[0.8rem] text-muted">{stats ? "No claim history in this range." : "-"}</p>}
      </div>
    </section>
  );
}
