...
const { listWorkbenchAgentCommands } = require("../../../../daemon/server/lib/workbench/commands/workbench-agent-command-registry.ts") as typeof import("../../../../daemon/server/lib/workbench/commands/workbench-agent-command-registry.ts");
...
test("hidden reload commands keep their raw operands out of thread rendering", () => {
  ...
  assert.equal(getThreadCommandDisplay({
    command: "wb daemon reload --server:mcp",
    commandActions: [], cwd: PROJECT_ROOT, projectRootPath: PROJECT_ROOT,
  }).claimedBy, null);
});
...
test("PowerShell ripgrep summaries do not treat an uppercase context value as the query", () => {
  const display = getThreadCommandDisplay({
    command: String.raw`"C:\Program Files\PowerShell\7\pwsh.exe" -Command 'rg -n -C 8 "rotate|selectedHarness|onHarness|HarnessIcon|harness" app/client/components/workbench.tsx | Select-Object -First 180'`,
    commandActions: [],
    cwd: PROJECT_ROOT,
    projectRootPath: PROJECT_ROOT,
  });
...
test("PowerShell ripgrep summaries preserve lowercase count flags as non-consuming", () => {
  const display = getThreadCommandDisplay({
    command: "pwsh -Command 'rg -n -c needle app/client/components/workbench.tsx'",
    commandActions: [],
    cwd: PROJECT_ROOT,
    projectRootPath: PROJECT_ROOT,
  });
...
test("typed ripgrep summaries exactly match the shell ripgrep presentation", () => {
  const cases = [
    {
      args: ["-n", "-C", "3", "needle|thread", "app/client/components/workbench.tsx"],
      command: String.raw`pwsh -Command 'rg -n -C 3 "needle|thread" app/client/components/workbench.tsx'`,
    },
    {
      args: ["-n", "-e", String.raw`needle\(thread`, "webapp/daemon"],
      command: String.raw`pwsh -Command 'rg -n -e "needle\(thread" webapp/daemon'`,
    },
    {
...
test("PowerShell numbered reads resolve a preceding literal path assignment", () => {
  const display = getThreadCommandDisplay({
    command: String.raw`"c:\\Program Files\\PowerShell\\7\\pwsh.exe" -Command '$p='"'"'app\\client\\workbench\\thread\\command-matchers\\workbench-cli.ts'"'"'; $c=Get-Content $p; $c[80..116]'`,
    commandActions: [],
    cwd: PROJECT_ROOT,
    projectRootPath: PROJECT_ROOT,
  });
  ...
  assert.deepEqual(pathOperands(display.summaryParts), [
    "app/client/workbench/thread/command-matchers/workbench-cli.ts",
  ]);
