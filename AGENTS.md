...
- Make long-lived daemon changes reload-capable in the same changeset. When a scoped reload or full restart will be needed, prominently tell the user which. You must be confident about what reloads will be required, this is a required step of inspection. Reloads and restarts are user-owned. Never include a permission request for them in agent work or plans, and definitely do not run them.
