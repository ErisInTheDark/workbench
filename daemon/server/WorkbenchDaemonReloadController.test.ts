/*
 * No production exports. Tests protect user-owned reload selection, dirt advancement, handoff, and hard-reload lifecycle.
 */
...
import WorkbenchDaemonReloadController from "./WorkbenchDaemonReloadController";
...
function dirtStub(options: {
  dirty?: Array<{ destructive: boolean; scope: DaemonReloadScope }>;
  events?: string[];
  listeners?: Set<() => void>;
}) {
  const events = options.events ?? [];
  return {
    beginReload: (scopes: readonly DaemonReloadScope[]) => events.push(`begin:${scopes.join(",")}`),
    completeReload: async (scopes: readonly DaemonReloadScope[]) => { events.push(`complete:${scopes.join(",")}`); },
...
test("all selects live dirt while destructive scopes require unsafe or an explicit selection", () => {
  const controller = new WorkbenchDaemonReloadController({
    dirt: dirtStub({ dirty: [
  ...
  assert.deepEqual(new WorkbenchDaemonReloadController({ dirt: dirtStub({}), executeBatch: async () => undefined }).resolveSelections({ all: true }, "cli"), []);
});
...
test("a replacement controller completes an in-flight batch without executing it twice", async () => {
  ...
  const first = new WorkbenchDaemonReloadController({
    dirt: dirtStub({ events }),
  ...
  const replacement = new WorkbenchDaemonReloadController({
