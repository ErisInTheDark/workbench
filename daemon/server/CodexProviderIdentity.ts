...
export async function admitProviderNotifications(
  ...
  if (turns.length) {
    ...
    await owners.threads.observeTurns(turns.map((turn) => ({
      kind: "turn", threadId, turnId: NativeTurnIdSchema.parse(turn.id), nativeTurnId: NativeTurnIdSchema.parse(turn.id),
  ...
  for (const { params } of notifications) {
    ...
    if (!owners.threads.findNativeTurn({ ...native, nativeTurnId: NativeTurnIdSchema.parse(params.turnId) })) {
      await owners.threads.resolveTurn({
        threadId: owners.threads.workbenchIdForNative(native), turnId: NativeTurnIdSchema.parse(params.turnId),
      });
    }
  }
  ...
  if (items.length) {
    const threadId = owners.threads.workbenchIdForNative(native);
    const admissions = items.map(({ turnId: reference, item }) => {
      const nativeTurnId = NativeTurnIdSchema.parse(reference);
...
export async function admitProviderThreads(
  ...
  await owners.threads.observeTurns(catalogs.flatMap(({ metadata, thread, identity, catalog }) => catalog.map((entry) => ({
    kind: "turn", threadId: identity.threadId, turnId: NativeTurnIdSchema.parse(entry.turnId),
    harnessId: metadata.native.harness, nativeLocation: metadata.native.nativeLocation,
    nativeThreadId: metadata.native.nativeThreadId, nativeTurnId: NativeTurnIdSchema.parse(entry.turnId),
...
export function mapProviderTurn(
  ...
  if (isPendingWorkbenchTurn(turn)) return turn;
  const nativeTurn = { ...native, nativeTurnId: NativeTurnIdSchema.parse(turn.id) };
  return {
    ...turn,
...
export function mapProviderThread(
  ...
  const identity = { ...native, nativeThreadId: NativeThreadIdSchema.parse(thread.id) };
  ...
  const history = readWorkbenchTurnHistory(thread)?.filter((entry) => !pending.has(entry.turnId)).map((entry) => {
    const turnId = owners.threads.workbenchTurnIdForNative({ ...identity, nativeTurnId: NativeTurnIdSchema.parse(entry.turnId) });
  ...
  if (typeof source === "object" && "subAgent" in source
    ...
    source = { subAgent: { thread_spawn: {
      ...source.subAgent.thread_spawn,
      parent_thread_id: owners.threads.workbenchIdForNative(owners.threads.knownNativeBinding(native.harness, NativeThreadIdSchema.parse(source.subAgent.thread_spawn.parent_thread_id))),
    } } };
  }
  return {
    ...thread,
    id: threadId,
    forkedFromId: null,
    parentThreadId: thread.parentThreadId === null ? null : owners.threads.workbenchIdForNative(owners.threads.knownNativeBinding(native.harness, NativeThreadIdSchema.parse(thread.parentThreadId))),
    source,
...
export function mapProviderNotification(
  ...
  const identity = { ...native, nativeThreadId: NativeThreadIdSchema.parse(params.threadId) };
  const threadId = owners.threads.workbenchIdForNative(identity);
  ...
  if (notification.method === "item/started" || notification.method === "item/completed") {
    const nativeTurn = { ...identity, nativeTurnId: NativeTurnIdSchema.parse(notification.params.turnId) };
...
export function mapProviderThreadItem(
  ...
  if (mapped.type === "subAgentActivity") {
    return {
      ...mapped,
      agentThreadId: owners.threads.workbenchIdForNative(owners.threads.knownNativeBinding(native.harness, NativeThreadIdSchema.parse(mapped.agentThreadId))),
    };
  }
  if (mapped.type === "collabAgentToolCall") {
    ...
    for (const [nativeThreadId, state] of Object.entries(mapped.agentsStates)) {
      const canonicalThreadId = owners.threads.workbenchIdForNative(owners.threads.knownNativeBinding(native.harness, NativeThreadIdSchema.parse(nativeThreadId)));
    ...
    return {
      ...mapped,
      senderThreadId: owners.threads.workbenchIdForNative(owners.threads.knownNativeBinding(native.harness, NativeThreadIdSchema.parse(mapped.senderThreadId))),
      receiverThreadIds: mapped.receiverThreadIds.map((nativeThreadId) => (
        owners.threads.workbenchIdForNative(owners.threads.knownNativeBinding(native.harness, NativeThreadIdSchema.parse(nativeThreadId)))
      )),
      agentsStates,
    };
  }
