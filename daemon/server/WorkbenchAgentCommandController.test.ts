...
test("dispatches token counting and scopes only managed-thread help by cwd", async () => {
  ...
  try {
    const request = async (args: string[], cwd: string, callerThreadId: string | null = null) => {
      ...
      if (callerThreadId) body.set("callerThreadId", callerThreadId);
      return await fetch(`${server.origin}/daemon/agent-command`, {
        body,
...
test("transcript CLI dispatch keeps Workbench target ids and rejects outside-root managed callers", async () => {
  ...
  try {
    for (const [cwd, expected] of [["C:/other", 400], ["C:/workbench", 200]] as const) {
      ...
      const response = await fetch(`${server.origin}/daemon/agent-command`, { method: "POST", body });
...
test("answers the private apply_patch hook from the active claim owner", async () => {
  ...
  const server = await startController(controller);
  try {
    const request = async (filePath: string) => await fetch(`${server.origin}/daemon/agent-command`, {
    ...
    const mixed = await fetch(`${server.origin}/daemon/agent-command`, {
...
test("returns Codex deny decisions for mismatched identity, malformed input, and claim-read failure", async () => {
  ...
  const server = await startController(controller);
  try {
    const send = async (body: string) => await fetch(`${server.origin}/daemon/agent-command`, {
      body,
      headers: { "Content-Type": "application/x-www-form-urlencoded" },
      method: "POST",
    });
...
test("reload admission releases the handler before terminal polling completes", async () => {
  ...
  try {
    ...
    const client = fetch(`${server.origin}/daemon/agent-command`, {
...
test("dirt and all share the live dirt snapshot while unsafe remains explicit", async () => {
  ...
  const server = await startController(controller);
  const run = async (args: string[]) => await fetch(`${server.origin}/daemon/agent-command`, {
