/*
 * Exports:
 * - ReloadNodeSourceDescriptor/ReloadNodeSourceState: generated live source ownership and dependant metadata.
 * - observeReloadNodeGraphSources: combine candidate-loaded module paths, root instruction reads, and explicit hostile-boundary patterns.
 * - activateReloadNodeSourceState/cancelReloadNodeSourceState: publish successful candidates or discard failed ones across module generations.
 * - readReloadNodeSourceState: read active metadata without exposing an uncommitted candidate.
 */
...
export type ReloadNodeSourceDescriptor = ReloadDirtSourceDescriptor & DaemonReloadScopeDescriptor;
...
function flatten(roots: readonly GraphNode[]) {
  const nodes = new Map<DaemonReloadScope, GraphNode>();
  const parents = new Map<DaemonReloadScope, Set<DaemonReloadScope>>();
  const visit = (node: GraphNode, parent: DaemonReloadScope | null) => {
...
function findDefiningModules(nodes: ReadonlyMap<DaemonReloadScope, GraphNode>) {
  const result = new Map<DaemonReloadScope, NodeModule>();
...
export function observeReloadNodeGraphSources<TContext, TObjects extends object, TNotification>(
  ...
  const pathsByScope = new Map<DaemonReloadScope, Set<string>>();
  const add = (scope: DaemonReloadScope, sourcePath: string) => {
  ...
  const cloned = new Map<DaemonReloadScope, GraphNode>();
  ...
  descriptors.push({
    access: "operator",
    description: "Restart the complete daemon process.",
    destructive: true,
    paths: [...(pathsByScope.get("server:process") ?? [])].sort(),
    safeAll: false,
    scope: "server:process",
  });
  const children = new Map<DaemonReloadScope, Set<DaemonReloadScope>>();
  ...
  sourceRegistry().pending = {
    dependantClosure(scopes) {
      ...
      const visit = (scope: DaemonReloadScope) => {
