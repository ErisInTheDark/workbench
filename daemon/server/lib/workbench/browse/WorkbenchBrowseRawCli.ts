/*
 * Exports:
 * - default WorkbenchBrowseRawCli: execute explicitly enabled raw Browse CLI compatibility commands in an isolated child process.
 */
...
export default class WorkbenchBrowseRawCli {
  ...
  async run(request: WorkbenchBrowseCommandRequest, signal?: AbortSignal): Promise<WorkbenchBrowseCommandResponse> {
    ...
    const entrypoint = path.join(appRoot, "server", "lib", "workbench", "browse", "run-browse-cli.mjs");
