/*
 * Exports:
 * - createWorkbenchReloadCommands: build hidden user-owned reload and dirt CLI contracts from the live dirt catalog.
 */
import { z } from "zod";
...
type ReloadAccess = DaemonReloadScopeDescriptor["access"];

function buildReloadRequest(input: { all?: boolean; scopes?: readonly DaemonReloadScope[]; unsafe?: boolean }) {
  return {
    ...postWorkbenchAgentCommand("/api/daemon/reload", {
      ...(input.all ? { all: true } : {}),
...
export function createWorkbenchReloadCommands(catalog: readonly DaemonReloadScopeDescriptor[], access: ReloadAccess) {
  ...
  const reload = defineWorkbenchAgentCommand({
    ...
    parseCliArgs(args) {
      ...
      const scopes = parsed.selections.length ? expandDaemonReloadScopes(parsed.selections) : undefined;
      resolveDaemonReloadSelections({ all: parsed.all, scopes, unsafe: parsed.unsafe }, catalog, access);
  ...
  const dirt = defineWorkbenchAgentCommand({
    ...
    buildRequest() { return getWorkbenchAgentCommand("/api/daemon/dirt", "reload-dirt"); },
  });
