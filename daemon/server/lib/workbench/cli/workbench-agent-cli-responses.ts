...
export function adaptWorkbenchAgentCliResponse({
  ...
  switch (request.responseKind) {
    ...
    case "daemon-reload":
      return adaptDaemonReload(payload, text);
    case "json":
...
function adaptDaemonReload(payload: Record<string, unknown> | null, fallback: string) {
  if (!payload) {
    return raw(fallback);
  }
  if (payload.state === "failed") {
    return failed(readError(payload) || "Daemon reload failed.");
  }
