/*
 * Exports:
 * - OpenCodeAppServerOptions: process factory, environment, clock, config, and logging ports.
 * - default OpenCodeAppServer: own external selection, managed startup, cooldown, disabled state, restart, and shutdown.
 */
...
export interface OpenCodeAppServerOptions {
  ...
  ensureConfig?: DaemonReloadableModules["opencodeWorkbenchInstructions"]["ensureOpenCodeWorkbenchConfigDirectory"];
  environment?: NodeJS.ProcessEnv;
  getReloadableModules: () => DaemonReloadableModules;
...
export default class OpenCodeAppServer {
  ...
  private readCooldownError() {
    ...
    if (this.failure.retryMode === "disabled") {
      ...
      return `${this.failure.message} OpenCode is optional and will stay disabled for this Workbench daemon process until it is restarted with a working OpenCode executable or OPENCODE_SERVER_URL.`;
    }
