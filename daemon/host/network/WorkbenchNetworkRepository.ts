/*
 * Exports:
 * - default WorkbenchNetworkRepository: own typed network configuration and membership transactions.
 */
...
export default class WorkbenchNetworkRepository {
  constructor(private readonly database: Pick<WorkbenchServiceRepository, "query" | "executeTransaction">) {}
  ...
  write(configuration: WorkbenchNetworkConfiguration, accompanying: readonly WorkbenchDatabaseMutation[] = []) {
    ...
    if (next.privateAccess) {
      ...
      if (next.privateAccess.role === "member") {
        statements.push(insertRow(tables.networkPrivateIssuer, {
          id: "singleton", role: "member",
          address: next.privateAccess.issuer.address,
          hostname: next.privateAccess.issuer.hostname,
        }));
      }
    }
    for (const member of next.members) {
    ...
    this.database.executeTransaction([...statements, ...accompanying]);
  }
