...
export default class ClaimedTestSelector {
  ...
  static async load(catalog: ProjectTestCatalog) {
    ...
    const server = require(path.join(catalog.root, "daemon/orchestrator/orchestrator-root-node.ts")) as { default: { roots: TestReloadNode[] } };
    const app = require(path.join(catalog.root, "app/runtime/app-root-node.ts")) as { default: { roots: TestReloadNode[] } };
