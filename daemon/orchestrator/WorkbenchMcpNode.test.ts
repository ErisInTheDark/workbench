...
for (const outcome of ["replacement", "retiring success", "restore", "admitted during startup"] as const) {
  test(`a pending command survives topology handoff with ${outcome}`, async () => {
    ...
    const context = {
      ...
      refreshWorkbenchPromptFiles: async () => {
        if (outcome === "restore") throw new Error("candidate prompt refresh failed");
    ...
    finishStarting();
    reload ??= host.reload(["server:topology"]);
    if (outcome === "restore") await assert.rejects(reload, /candidate prompt refresh failed/u);
    else await reload;
    await checked;
    await host.dispose();
  });
