...
test("canonical asset requests use only their thread's retained native folders", async () => {
  ...
  try {
    for (const reference of [owner.threadId, "native-owner", encoded]) {
      const response = await request(controller, `/orchestrator/transcript-assets/codex/${reference}/${asset}`);
      assert.equal(response.statusCode, 200);
      assert.deepEqual([...response.body], [4, 5, 6]);
    }
    assert.equal((await request(controller, `/orchestrator/transcript-assets/codex/${other.threadId}/${asset}`)).statusCode, 404);
