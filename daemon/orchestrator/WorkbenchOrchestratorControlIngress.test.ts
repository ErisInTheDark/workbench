/*
 * Keywords: websocket, health, reload, admission, failure, process.
...
import WorkbenchOrchestratorControlIngress, { type WorkbenchOrchestratorControlIngressOptions } from "./WorkbenchOrchestratorControlIngress";

function fixture(overrides: Partial<WorkbenchOrchestratorControlIngressOptions> = {}, send?: BridgeClient["send"]) {
  ...
  const ingress = new WorkbenchOrchestratorControlIngress({
...
test("health bypasses blocked feature work and validates its own params", async () => {
  ...
  await target.handle("workbench/orchestrator/health");
  assert.deepEqual(target.responses, [{ id: 7, result: { ok: true } }]);
  const invalid = fixture();
  await invalid.handle("workbench/orchestrator/health", { unexpected: true });
...
test("reload resolves the current owner and starts only after its response is sent", async () => {
  ...
  generation = 2;
  const handling = target.handle("workbench/orchestrator/reload", { scopes: ["server:process"] });
  await sent;
...
test("failed response delivery cancels reload admission instead of starting it", async () => {
  ...
  await assert.rejects(target.handle("workbench/orchestrator/reload", { scopes: ["server:core"] }), /socket write failed/u);
