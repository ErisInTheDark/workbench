...
function fakeLog(lines: string[]) {
  return {
    close() {},
    createLineStream(_domain: "orchestrator", _error: boolean, onLine: () => void) {
      return {
        flush() {},
    ...
    error(_domain: "runner", message: string) { lines.push(message); },
    line(_domain: "runner", message: string) { lines.push(message); },
  };
}
...
test("retries failed health once before killing owned ports at the silence deadline", async (context) => {
  ...
  let kills = 0;
  let spawns = 0;
  const runner = new WorkbenchOrchestratorRunner({
    environment: {
...
test("stop exits a paused runner without spawning or deleting the sentinel", async (context) => {
  ...
  context.after(async () => await rm(root, { force: true, recursive: true }));
  const sentinel = path.join(root, ".workbench", "orchestrator-loop.pause");
