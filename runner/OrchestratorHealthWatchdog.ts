/*
 * Exports:
 * - OrchestratorHealthWatchdogAction/OrchestratorHealthProbeToken: explicit runner liveness decisions and stale-probe fence. Keywords: health, probe, restart.
 * - default OrchestratorHealthWatchdog: own silence age, retry stage, in-flight probe, and restart admission. Keywords: watchdog, lifecycle, monotonic.
 */
export type OrchestratorHealthProbeToken = {
  attempt: 1 | 2;
  generation: number;
};

export type OrchestratorHealthWatchdogAction =
  | { delayMs: number; kind: "wait" }
  | { kind: "probe"; token: OrchestratorHealthProbeToken }
  | { kind: "restart" };

export default class OrchestratorHealthWatchdog {
  ...
  constructor(
    ...
    if (!Number.isFinite(idleTimeoutMs) || idleTimeoutMs <= 0) {
      throw new Error("Orchestrator idle timeout must be positive.");
    }
  ...
  nextAction(now: number): OrchestratorHealthWatchdogAction {
  ...
  completeProbe(token: OrchestratorHealthProbeToken, succeeded: boolean, completedAt: number) {
    if (
  ...
  private beginProbe(attempt: 1 | 2): OrchestratorHealthWatchdogAction {
