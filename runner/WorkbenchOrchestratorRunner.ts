/*
 * Exports:
 * - default WorkbenchOrchestratorRunner: own daemon child startup, logs, pause, health recovery, port cleanup, restart, and shutdown. Keywords: runner, supervisor, lifecycle, health.
 * Local mechanics:
...
import WorkbenchOrchestratorHealthClient from "./WorkbenchOrchestratorHealthClient.ts";
...
interface RunnerLog {
  close(): void;
  createLineStream(domain: "orchestrator", error: boolean, onLine: () => void): {
    flush(): void;
    write(chunk: string | Buffer): void;
  };
  error(domain: "runner", message: string): void;
  line(domain: "runner", message: string): void;
}

interface WorkbenchOrchestratorRunnerOptions {
  environment?: NodeJS.ProcessEnv;
  healthClient?: Pick<WorkbenchOrchestratorHealthClient, "probe">;
  ...
  spawnOrchestrator?: (daemonDirectoryPath: string, environment: NodeJS.ProcessEnv) => ChildProcess;
...
class RunnerLogFile implements RunnerLog {
  ...
  constructor(logFilePath: string) {
    ...
    const write = (stream: NodeJS.WriteStream, value: string) => {
      if (this.closed) throw new Error("Orchestrator runner log is closed.");
      stream.write(value);
  ...
  error(domain: "runner", message: string) {
    this.logger.error(domain, message);
  }

  line(domain: "runner", message: string) {
...
function defaultSpawnOrchestrator(daemonDirectoryPath: string, environment: NodeJS.ProcessEnv) {
  return spawn(executable("pnpm"), ["dev:orchestrator"], {
    cwd: daemonDirectoryPath,
    env: { ...environment, WORKBENCH_ORCHESTRATOR_LOOP: "1", FORCE_COLOR: "1" },
    stdio: ["ignore", "pipe", "pipe"],
    windowsHide: true,
...
export default class WorkbenchOrchestratorRunner {
  private readonly daemonDirectoryPath: string;
  private readonly directLogger = new WorkbenchProcessLogger();
  private readonly environment: NodeJS.ProcessEnv;
  private readonly healthClient: Pick<WorkbenchOrchestratorHealthClient, "probe">;
  ...
  private readonly spawnOrchestrator: (daemonDirectoryPath: string, environment: NodeJS.ProcessEnv) => ChildProcess;
  ...
  constructor(options: WorkbenchOrchestratorRunnerOptions) {
    ...
    this.pauseSentinelPath = path.join(this.projectRootPath, ".workbench", "orchestrator-loop.pause");
    this.environment = options.environment ?? process.env;
    this.topology = options.topology ?? deriveWorkbenchRuntimeTopology(this.environment);
    this.healthClient = options.healthClient ?? new WorkbenchOrchestratorHealthClient();
    this.loggerFactory = options.loggerFactory ?? ((logFilePath) => new RunnerLogFile(logFilePath));
    this.now = options.now ?? (() => performance.now());
    this.runCommand = options.runCommand ?? defaultRunCommand;
    this.sleep = options.sleep ?? abortableSleep;
    this.spawnOrchestrator = options.spawnOrchestrator ?? defaultSpawnOrchestrator;
  ...
  async run({ dryRun = false }: { dryRun?: boolean } = {}) {
    if (dryRun) {
      this.directLogger.line("runner", `dry run: Workbench would kill listeners on configured ports: ${this.topology.listeners.map(({ port }) => port).join(" ")}`);
      return;
    }
    ...
    while (!this.stopping) {
      ...
      try {
        ...
        log.line("runner", `logging complete orchestrator output to: ${logFilePath}`);
        ...
        restartNumber += 1;
        log.line("runner", `restarting Workbench orchestrator in ${this.restartDelayMs / 1_000} seconds after child status ${result.exitCode ?? result.signal ?? "unknown"}.`);
        try {
  ...
  async stop(reason = "Orchestrator runner stopped.") {
  ...
  private async runChild(log: RunnerLog) {
    ...
    const child = this.spawnOrchestrator(this.daemonDirectoryPath, this.environment);
    this.activeChild = child;
    const exited = childResult(child);
    const watchdog = new OrchestratorHealthWatchdog(this.idleTimeoutMs, this.now());
    const wake = new WakeSignal();
    const observeOutput = () => {
      watchdog.observeOutput(this.now());
      wake.wake();
    };
    const stdout = log.createLineStream("orchestrator", false, observeOutput);
    const stderr = log.createLineStream("orchestrator", true, observeOutput);
    ...
    try {
      while (!this.stopping) {
        ...
        if (action.kind === "restart") {
          log.error("runner", `no orchestrator output or successful WebSocket health response was received for ${this.logIdleTimeoutSeconds} seconds; killing owned ports for restart.`);
          await this.killOwnedPorts(log);
          return await exited;
        }
        if (action.kind === "wait") {
          ...
          if (winner.kind !== "timer" && !waitAbort.signal.aborted) {
            waitAbort.abort(new Error("Orchestrator watchdog wait became stale."));
            await sleeping;
          }
        ...
        log.line("runner", `probing orchestrator WebSocket health after ${probeAfterSeconds} seconds without output (attempt ${action.token.attempt}/2).`);
        ...
        if (winner.kind !== "probe") {
          probeAbort.abort(new Error("Orchestrator health probe became stale."));
          await probe;
          if (winner.kind === "exit") return winner.result;
          continue;
        }
        if (!watchdog.completeProbe(action.token, winner.succeeded, this.now())) continue;
        if (winner.succeeded) log.line("runner", "orchestrator WebSocket health probe succeeded.");
        else log.error("runner", `orchestrator WebSocket health probe failed (${action.token.attempt}/2): ${winner.message.slice(0, 500)}`);
      }
      return await exited;
    } finally {
      abort.abort(new Error("Orchestrator child supervision ended."));
      stdout.flush();
      stderr.flush();
  ...
  private logConfiguration(log: RunnerLog) {
    log.line("runner", "starting Workbench orchestrator restart loop.");
    log.line("runner", "command: pnpm dev:orchestrator");
    log.line("runner", `working directory: ${this.daemonDirectoryPath}`);
    log.line("runner", `owned ports: ${this.topology.listeners.map(({ port }) => port).join(" ")}`);
    log.line("runner", `log directory: ${this.logDirectoryPath}`);
    log.line("runner", `log rotation: more than ${this.maxLogLines} lines`);
    log.line("runner", `log retention: ${this.maxLogFiles} files`);
    log.line("runner", `restart delay: ${this.restartDelayMs / 1_000} seconds`);
    log.line("runner", `inactivity recovery: WebSocket probes at ${this.logIdleTimeoutSeconds / 2} and ${this.logIdleTimeoutSeconds * 3 / 4} seconds; restart after ${this.logIdleTimeoutSeconds} seconds`);
    log.line("runner", `pause sentinel: ${this.pauseSentinelPath}`);
    log.line("runner", "press Ctrl+C to stop.");
  }
  ...
  private async killOwnedPorts(log: RunnerLog | null) {
    for (const { port } of this.topology.listeners) {
      ...
      for (const line of outputLines(result.stdout)) (log ?? this.directLogger).line("runner", line);
      for (const line of outputLines(result.stderr)) (log ?? this.directLogger).error("runner", line);
      if (result.error) throw result.error;
  ...
  private async waitWhilePaused(log: RunnerLog) {
    ...
    while (!this.stopping && await this.exists(this.pauseSentinelPath)) {
      if (!announced) {
        log.line("runner", `orchestrator restart loop paused by sentinel: ${this.pauseSentinelPath}`);
        announced = true;
      }
      try {
        await this.sleep(this.restartDelayMs, this.stopAbort.signal);
    ...
    if (announced && !this.stopping) log.line("runner", "orchestrator restart loop pause released.");
  }
  ...
  private async createLogFile(restartNumber: number) {
    ...
    const filePath = path.join(
      ...
      `workbench-orchestrator-${timestamp}-${process.pid}-${String(restartNumber).padStart(4, "0")}.log`,
    );
  ...
  private async logFiles() {
    ...
    return names
      .filter((name) => /^workbench-orchestrator-.*\.log$/u.test(name))
      .sort()
