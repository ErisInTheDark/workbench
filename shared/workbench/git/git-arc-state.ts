...
export function applyGitClaimChanges(existing: readonly string[], changes: GitArcClaimChanges) {
  ...
  if (removals.length && !changes.inherit) throw new Error("Removing claims requires inheritance.");
  const inherited = changes.inherit ? existing : [];
  const unknown = removals.filter((candidate) => !inherited.includes(candidate));
  if (unknown.length) throw new Error(`Removed paths must exactly match inherited entries: ${unknown.join(", ")}`);
  ...
  if (contradictory.length) throw new Error(`Conflicting claim operations: ${[...new Set(contradictory)].join(", ")}`);
