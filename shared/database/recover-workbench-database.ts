...
import {
  preserveWorkbenchDatabaseBackup, readWorkbenchDatabaseBackups, restoreWorkbenchDatabaseBackup,
...
export default async function recoverWorkbenchDatabase(
  ...
  try {
    ...
    archive = await preserveWorkbenchDatabaseBackup(database, path.join(directory, "failed-upgrades"));
  ...
  console.info(`[database] restored schema ${schema.currentVersion} from schema ${installedVersion}; failed-upgrade archive ${safePath}`);
}
